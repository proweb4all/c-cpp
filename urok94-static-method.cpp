...
class Human{
	public:
		...
		static void chNameStatic(Human & human, string name){
			human.name = name;
		}
		void chNameNoStatic(string name){
			this->name = name;
		}
...
//	D.printP();
//	D.printAlarm();
//	cout << "Людей " << Human::getCount() << endl;
//	Human NN;
//	cout << "Людей " << Human::getCount() << endl;
//	NN.drawP(D);
//	NN.upP(D);
	...
	cout << "Людей " << Human::getCount() << endl;
	MM.drawP(B);
	Human::chNameStatic(MM,"Вася");
	MM.drawP(B);
	MM.chNameStatic(MM,"Петя");
	MM.drawP(B);
	MM.chNameNoStatic("Коля");
	MM.drawP(B);
